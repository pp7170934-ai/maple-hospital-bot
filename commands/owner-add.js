...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('owner-add')
    .setDescription('Add an in-game owner (Server Owner only)')
  ...
  async execute(interaction) {
    ...
    const data = getData();
    const already = data.owners.some(o => o.discordId === discordId);
    if (already) {
      return interaction.reply({ content: `That user is already an owner.`, ephemeral: true });
    }
